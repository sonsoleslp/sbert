...
\`"(a)"\`, \`"(iv)"\`, \`"IV."\`, \`"V."\` — from each document. The
\`numbers\`, \`roman_numerals\`, and \`section_numbers\` arguments of
\[topics()\] clean only the extracted \*terms\*; this cleans the source
\*text\*, so markers no longer over-fragment \[segment()\], skew the
embeddings, or surface in \[representatives()\]\[representatives\]. Run
it once, before embedding or segmentation, as a companion to
\[dedupe()\].
...
period (\`"1."\`, \`"12."\`); a parenthesized one- or two-digit number,
single letter, or Roman numeral up to 39 (\`"(1)"\`, \`"(a)"\`,
\`"(iv)"\`); and a bare upper-case Roman-numeral section marker with a
period (\`"II."\`, \`"IV."\`). A multi-letter numeral is removed
anywhere; a single-letter one (\`"V."\`, \`"X."\`) only at the start of
the text, so a mid-sentence initial (\`"by V. Nabokov"\`), unit (\`"5
V."\`), and lower-case \`"v."\` (versus) are kept.
